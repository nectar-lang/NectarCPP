...
NectarCore::VAR __Nectar_Object_Keys(NectarCore::VAR _var)
{
	...
	if (_var.type == NectarCore::Enum::Type::Array)
		_obj = &((NectarCore::Class::Array *)_var.data.ptr)->object;
	else
		_obj = &((NectarCore::Class::Object *)_var.data.ptr)->object;
...
NectarCore::VAR operator+(NectarCore::VAR _left, int right)
{
	if (_left.type == NectarCore::Enum::Type::String)
		return (std::string)_left + __Nectar_DOUBLE_TO_STRING(right);
	else
		return (double)_left + right;
}
...
{
	if (_left.type == NectarCore::Enum::Type::Number)
		_left.data.number += right;
	else
	{
		std::string _str = (std::string)_left;
		_str += right;
...
NectarCore::VAR operator||(NectarCore::VAR _left, int right)
{
	if (_left.type != NectarCore::Enum::Type::Undefined)
		return _left;
	else
		return right;
}
